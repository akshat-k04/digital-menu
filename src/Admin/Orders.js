      ...
      <ul className="order-list">
        {orders.map((order) => (
          <li key={order._id} className="order-item">
            ...
            <p className="order-created-at"><strong>Created At:</strong> {new Date(order.createdAt).toLocaleString()}</p>
            <h3 className="order-items-title">Items:</h3>
            <ul className="order-items-list">
              {order.items.map((item, index) => (
                <li key={index} className="order-item-detail">
                  <p className="order-item-name"><strong>Dish:</strong> {item.menuItem}</p>
                  <p className="order-item-quantity"><strong>Quantity:</strong> {item.quantity}</p>
                  {item.specialInstructions && (
                    <p className="order-item-instructions"><strong>Instructions:</strong> {item.specialInstructions}</p>
                  )}
                </li>
              ))}
            </ul>
            <button onClick={() => handleDeleteOrder(order._id)} className="delete-order-button">Delete</button>
          </li>
        ))}
      </ul>
    </div>
